...
@section('action-label', 'Editar Embarque')
        ...
        <div class="col-md-12">
            ...
            <section class="tile">
                ...
                <div class="tile-header dvd dvd-btm">
                    <h1 class="custom-font"><strong>Editar</strong> Lançamento de Embarque</h1>
                </div>
                <!-- /tile header -->
                <!-- tile body -->
                <div class="tile-body">
                    <form class="form-horizontal" role="form" method="post" action="{{ url('embarque/' . $embarque->id) }}">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <select name="status" class="form-control">
                                            <option value="ativo" {{ old('status', $status) == 'ativo' ? 'selected' : '' }}>Ativo</option>
                                            <option value="liquidado" {{ old('status', $status) == 'liquidado' ? 'selected' : '' }}>Liquidado</option>
                                            <option value="encerrado" {{ old('status', $status) == 'encerrado' ? 'selected' : '' }}>Encerrado</option>
                                            <option value="cancelado" {{ old('status', $status) == 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" name="entrega" class="form-control" value="{{ $embarque->entrega }}" readonly>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" name="nota_fiscal" class="form-control" value="{{ old('nota_fiscal', $embarque->nota_fiscal) }}">
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-2">
                                        <div style="padding-top: 7px; text-align: right">
                                            <span>Saldo:</span>
                                            <span id="saldo">{{ old('quantidade') ? $saldo - old('quantidade') : number_format($saldo, 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                </div>

                                @if($embarque->descontos->count() > 0)
                                    @foreach($embarque->descontos as $key => $desconto)
                                        @include('embarque.includes.desconto', [
                                          'count' => $key,
                                          'tipoDesconto' => $desconto->tipo,
                                          'valorDesconto' => str_replace(',', '', number_format(str_replace(',', '.', $desconto->valor), 2, ',', '.'))
                                        ])
                                    @endforeach
                                @else
                                    @include('embarque.includes.desconto', [
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <div class='input-group date' id='data_embarque'>
                                            <input type="text" name="data_embarque" class="form-control"  value="{{ old('data_embarque', $embarque->dataEmbarqueFormatado()) }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <div class='input-group date'>
                                            <input type="text" name="data_pagamento" class="form-control"  value="{{ old('data_pagamento', $embarque->dataPagamentoFormatado()) }}" id="data_pagamento">
                                            <span class="input-group-addon">
                                                <span class="fa fa-calendar"></span>
                                            </span>
                                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5">
                                        <input type="text" name="observacao" class="form-control" value="{{ old('observacao', $embarque->observacao) }}">
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#quantidade').on('keyup', function() {
            ...
            var quantidadePedido = parseInt('{{ $quantidade }}');
            var saldo = quantidadePedido - quantidade;

            if(isNaN(saldo)) {
                $('#saldo').text(quantidadePedido);
            }
            else {
