        ...
        <div class="col-md-12">
            ...
            <section class="tile">
                ...
                <div class="tile-header dvd dvd-btm">
                    <h1 class="custom-font"><strong>Novo</strong> Produto</h1>
                </div>
                <!-- /tile header -->
                <!-- tile body -->
                <div class="tile-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select tabindex="3" name="product[supplier_id]" class="chosen-select" id="company">
                                    <option value="" {{ old('product.supplier_id') == '' ? 'selected' : '' }}>Selecionar</option>

                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}"{{ ($company->id == old('product.supplier_id'))?' selected':''}}>
                                            {{ $company->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-10">
                                <select id="category" name="product[category_id]" class="chosen-select">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}"{{ ($category->id == old('product.category_id'))?' selected':''}}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
